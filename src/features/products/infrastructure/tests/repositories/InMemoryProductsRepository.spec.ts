...
import { InMemoryProductsRepository } from "../../InMemory/InMemoryProductsRepository";
...
describe("ProductsInMemoryRepository unit tests", () => {
    ...
    describe("applySort", () => {
        it("should sort items by created_at when sort param is null", async () => {
            ...
            const items = [
                ProductsDataBuilder({ name: "c", createdAt: created_at }),
                ProductsDataBuilder({
                    name: "a",
                    createdAt: new Date(created_at.getTime() + 100),
                }),
                ProductsDataBuilder({
                    name: "b",
                    createdAt: new Date(created_at.getTime() + 200),
                }),
            ];
            sut.items.push(...items);
